...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        ...
        $stmt = $pdo->prepare("SELECT admin_password FROM elms_admin WHERE admin_id = ?");
        ...
        if (password_verify($current_password, $user['admin_password'])) {
            ...
            $stmt = $pdo->prepare("UPDATE elms_admin SET admin_password = ? WHERE admin_id = ?");
...
<div class="row">
    <div class="col-md-4">
        ...
        if(isset($errors) && count($errors) > 0){
            echo getMsg('alert', '<ul class="list-unstyle"><li>'.implode('</li><li>', $errors).'</li></ul>');
        }
